// Forward declarations for org.apache.poi.ss.formula.ptg
#pragma once


namespace org
{
    namespace apache
    {
        namespace poi
        {
            namespace ss
            {
                namespace formula
                {
                    namespace ptg
                    {
                        class AbstractFunctionPtg;
                        class AddPtg;
                        class Area2DPtgBase;
                        class Area3DPtg;
                        class Area3DPxg;
                        class AreaErrPtg;
                        struct AreaI;
                        class AreaI_OffsetArea;
                        class AreaNPtg;
                        class AreaPtg;
                        class AreaPtgBase;
                        class ArrayPtg;
                        class ArrayPtg_Initial;
                        class AttrPtg;
                        class AttrPtg_SpaceType;
                        class BoolPtg;
                        class ConcatPtg;
                        class ControlPtg;
                        class Deleted3DPxg;
                        class DeletedArea3DPtg;
                        class DeletedRef3DPtg;
                        class DividePtg;
                        class EqualPtg;
                        class ErrPtg;
                        class ExpPtg;
                        class ExternSheetNameResolver;
                        class FuncPtg;
                        class FuncVarPtg;
                        class GreaterEqualPtg;
                        class GreaterThanPtg;
                        class IntPtg;
                        class IntersectionPtg;
                        class LessEqualPtg;
                        class LessThanPtg;
                        class MemAreaPtg;
                        class MemErrPtg;
                        class MemFuncPtg;
                        class MissingArgPtg;
                        class MultiplyPtg;
                        class NamePtg;
                        class NameXPtg;
                        class NameXPxg;
                        class NotEqualPtg;
                        class NumberPtg;
                        class OperandPtg;
                        class OperationPtg;
                        class ParenthesisPtg;
                        class PercentPtg;
                        class PowerPtg;
                        class Ptg;
                        struct Pxg;
                        struct Pxg3D;
                        class RangePtg;
                        class Ref2DPtgBase;
                        class Ref3DPtg;
                        class Ref3DPxg;
                        class RefErrorPtg;
                        class RefNPtg;
                        class RefPtg;
                        class RefPtgBase;
                        class ScalarConstantPtg;
                        class StringPtg;
                        class SubtractPtg;
                        class TblPtg;
                        class UnaryMinusPtg;
                        class UnaryPlusPtg;
                        class UnionPtg;
                        class UnknownPtg;
                        class ValueOperatorPtg;
                    } // ptg
                } // formula
            } // ss
        } // poi
    } // apache
} // org
