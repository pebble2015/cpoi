// Generated from /POI/java/org/apache/poi/hssf/record/chart/FrameRecord.java

#pragma once

#include <fwd-POI.hpp>
#include <java/lang/fwd-POI.hpp>
#include <org/apache/poi/hssf/record/fwd-POI.hpp>
#include <org/apache/poi/hssf/record/chart/fwd-POI.hpp>
#include <org/apache/poi/util/fwd-POI.hpp>
#include <org/apache/poi/hssf/record/StandardRecord.hpp>
#include <java/lang/Cloneable.hpp>

struct default_init_tag;

class org::apache::poi::hssf::record::chart::FrameRecord final
    : public ::org::apache::poi::hssf::record::StandardRecord
    , public ::java::lang::Cloneable
{

public:
    typedef ::org::apache::poi::hssf::record::StandardRecord super;
    static constexpr int16_t sid { int16_t(4146) };

private:
    static ::org::apache::poi::util::BitField* autoSize_;
    static ::org::apache::poi::util::BitField* autoPosition_;
    int16_t field_1_borderType {  };

public:
    static constexpr int16_t BORDER_TYPE_REGULAR { int16_t(0) };
    static constexpr int16_t BORDER_TYPE_SHADOW { int16_t(1) };

private:
    int16_t field_2_options {  };
protected:
    void ctor();
    void ctor(::org::apache::poi::hssf::record::RecordInputStream* in);

public:
    ::java::lang::String* toString() override;
    void serialize(::org::apache::poi::util::LittleEndianOutput* out) override;

public: /* protected */
    int32_t getDataSize() override;

public:
    int16_t getSid() override;
    FrameRecord* clone() override;
    int16_t getBorderType();
    void setBorderType(int16_t field_1_borderType);
    int16_t getOptions();
    void setOptions(int16_t field_2_options);
    void setAutoSize(bool value);
    bool isAutoSize();
    void setAutoPosition(bool value);
    bool isAutoPosition();

    // Generated
    FrameRecord();
    FrameRecord(::org::apache::poi::hssf::record::RecordInputStream* in);
protected:
    FrameRecord(const ::default_init_tag&);


public:
    static ::java::lang::Class *class_();
    static void clinit();
    int32_t serialize(int32_t offset, ::int8_tArray* data);
    ::int8_tArray* serialize();

private:
    static ::org::apache::poi::util::BitField*& autoSize();
    static ::org::apache::poi::util::BitField*& autoPosition();
    virtual ::java::lang::Class* getClass0();
};
