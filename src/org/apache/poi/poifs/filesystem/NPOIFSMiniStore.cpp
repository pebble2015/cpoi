...
poi::poifs::filesystem::NPOIFSMiniStore::NPOIFSMiniStore(const ::default_init_tag&)
    : super(*static_cast< ::default_init_tag* >(0))
{
    clinit();
}

poi::poifs::filesystem::NPOIFSMiniStore::NPOIFSMiniStore(NPOIFSFileSystem* filesystem, ::poi::poifs::property::RootProperty* root, ::java::util::List* sbats, ::poi::poifs::storage::HeaderBlock* header) 
    : NPOIFSMiniStore(*static_cast< ::default_init_tag* >(0))
{
    ctor(filesystem,root,sbats,header);
}

void poi::poifs::filesystem::NPOIFSMiniStore::ctor(NPOIFSFileSystem* filesystem, ::poi::poifs::property::RootProperty* root, ::java::util::List* sbats, ::poi::poifs::storage::HeaderBlock* header)
{
    super::ctor();
...
java::nio::ByteBuffer* poi::poifs::filesystem::NPOIFSMiniStore::getBlockAt(int32_t offset) /* throws(IOException) */
{
    auto byteOffset = offset * ::poi::poifs::common::POIFSConstants::SMALL_BLOCK_SIZE;
    ...
    npc(miniBuffer)->limit(::poi::poifs::common::POIFSConstants::SMALL_BLOCK_SIZE);
    return miniBuffer;
}

java::nio::ByteBuffer* poi::poifs::filesystem::NPOIFSMiniStore::createBlockIfNeeded(int32_t offset) /* throws(IOException) */
{
    auto firstInStore = false;
    if(npc(_mini_stream)->getStartBlock() == ::poi::poifs::common::POIFSConstants::END_OF_CHAIN) {
        firstInStore = true;
    }
    if(!firstInStore) {
        try {
    ...
    if(firstInStore) {
        ...
        while (true) {
            ...
            if(next == ::poi::poifs::common::POIFSConstants::END_OF_CHAIN) {
                break;
            }
            block = next;
        }
    ...
    npc(_filesystem)->setNextBlock(newBigBlock, ::poi::poifs::common::POIFSConstants::END_OF_CHAIN);
    return createBlockIfNeeded(offset);
}

poi::poifs::storage::BATBlock_BATBlockAndIndex* poi::poifs::filesystem::NPOIFSMiniStore::getBATBlockAndIndex(int32_t offset)
{
    return ::poi::poifs::storage::BATBlock::getSBATBlockAndIndex(offset, _header, _sbat_blocks);
}

int32_t poi::poifs::filesystem::NPOIFSMiniStore::getNextBlock(int32_t offset)
{
    auto bai = getBATBlockAndIndex(offset);
    return npc(npc(bai)->getBlock())->getValueAt(npc(bai)->getIndex());
}

void poi::poifs::filesystem::NPOIFSMiniStore::setNextBlock(int32_t offset, int32_t nextBlock)
{
    auto bai = getBATBlockAndIndex(offset);
    npc(npc(bai)->getBlock())->setValueAt(npc(bai)->getIndex(), nextBlock);
}

int32_t poi::poifs::filesystem::NPOIFSMiniStore::getFreeBlock() /* throws(IOException) */
{
    ...
    for (auto i = int32_t(0); i < npc(_sbat_blocks)->size(); i++) {
        auto sbat = java_cast< ::poi::poifs::storage::BATBlock* >(npc(_sbat_blocks)->get(i));
        if(npc(sbat)->hasFreeSectors()) {
            for (auto j = int32_t(0); j < sectorsPerSBAT; j++) {
                auto sbatValue = npc(sbat)->getValueAt(j);
                if(sbatValue == ::poi::poifs::common::POIFSConstants::UNUSED_BLOCK) {
                    return offset + j;
                }
            }
        }
    ...
    auto newSBAT = ::poi::poifs::storage::BATBlock::createEmptyBATBlock(npc(_filesystem)->getBigBlockSizeDetails(), false);
    ...
    if(npc(_header)->getSBATCount() == 0) {
        ...
        while (true) {
            ...
            if(nextBat == ::poi::poifs::common::POIFSConstants::END_OF_CHAIN) {
                break;
            }
            batOffset = nextBat;
        }
    ...
    npc(_filesystem)->setNextBlock(batForSBAT, ::poi::poifs::common::POIFSConstants::END_OF_CHAIN);
    npc(_sbat_blocks)->add(static_cast< ::java::lang::Object* >(newSBAT));
    return offset;
}

poi::poifs::filesystem::BlockStore_ChainLoopDetector* poi::poifs::filesystem::NPOIFSMiniStore::getChainLoopDetector() /* throws(IOException) */
{
    return new BlockStore_ChainLoopDetector(this, npc(_root)->getSize());
}

int32_t poi::poifs::filesystem::NPOIFSMiniStore::getBlockStoreBlockSize()
{
    return ::poi::poifs::common::POIFSConstants::SMALL_BLOCK_SIZE;
}

void poi::poifs::filesystem::NPOIFSMiniStore::syncWithDataSource() /* throws(IOException) */
{
    auto blocksUsed = int32_t(0);
    for (auto _i = npc(_sbat_blocks)->iterator(); _i->hasNext(); ) {
        ::poi::poifs::storage::BATBlock* sbat = java_cast< ::poi::poifs::storage::BATBlock* >(_i->next());
        {
            auto block = npc(_filesystem)->getBlockAt(npc(sbat)->getOurBlockIndex());
            ::poi::poifs::storage::BlockAllocationTableWriter::writeBlock(sbat, block);
            if(!npc(sbat)->hasFreeSectors()) {
...
java::lang::Class* poi::poifs::filesystem::NPOIFSMiniStore::class_()
{
    static ::java::lang::Class* c = ::class_(u"org.apache.poi.poifs.filesystem.NPOIFSMiniStore", 47);
    return c;
}

java::lang::Class* poi::poifs::filesystem::NPOIFSMiniStore::getClass0()
{
    return class_();
}
