...
poi::poifs::filesystem::DocumentFactoryHelper::DocumentFactoryHelper(const ::default_init_tag&)
    : super(*static_cast< ::default_init_tag* >(0))
{
    clinit();
}

poi::poifs::filesystem::DocumentFactoryHelper::DocumentFactoryHelper()
    : DocumentFactoryHelper(*static_cast< ::default_init_tag* >(0))
{
    ctor();
}

java::io::InputStream* poi::poifs::filesystem::DocumentFactoryHelper::getDecryptedStream(NPOIFSFileSystem* fs, ::java::lang::String* password) /* throws(IOException) */
{
    clinit();
    auto info = new ::poi::poifs::crypt::EncryptionInfo(fs);
    auto d = ::poi::poifs::crypt::Decryptor::getInstance(info);
    try {
        auto passwordCorrect = false;
        if(password != nullptr && npc(d)->verifyPassword(password)) {
            passwordCorrect = true;
        }
        if(!passwordCorrect && npc(d)->verifyPassword(::poi::poifs::crypt::Decryptor::DEFAULT_PASSWORD())) {
            passwordCorrect = true;
        }
        if(passwordCorrect) {
            ...
            if(password != nullptr)
                throw new ::poi::EncryptedDocumentException(u"Password incorrect"_j);
            else
                throw new ::poi::EncryptedDocumentException(u"The supplied spreadsheet is protected, but no password was supplied"_j);
        }
...
bool poi::poifs::filesystem::DocumentFactoryHelper::hasOOXMLHeader(::java::io::InputStream* inp) /* throws(IOException) */
{
    clinit();
...
java::lang::Class* poi::poifs::filesystem::DocumentFactoryHelper::class_()
{
    static ::java::lang::Class* c = ::class_(u"org.apache.poi.poifs.filesystem.DocumentFactoryHelper", 53);
    return c;
}

java::lang::Class* poi::poifs::filesystem::DocumentFactoryHelper::getClass0()
{
    return class_();
}
